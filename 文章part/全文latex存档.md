...
当用户输入"我是大学生……续航一定要好……看视频写论文"时，\texttt{MultiCategoryAgent} 将请求分发至 \texttt{LaptopAgent}，系统命中 \texttt{light\_office} 关键字配置，将 \texttt{Battery\_Score} 设为排序主轴，数据库最终返回 MacBook Air 与联想小新两款机型。输出的雷达图以"续航"和"便携"为长半轴，直观印证了推荐理由，使用户无需逐条比对参数即可建立选购判断。
![alt text](laptop.png)
![alt text](laptop2.png)
% ------------------------------------------------------------
\subsection{案例 2：极致性能的游戏发烧场景（全场景生态组合）}
% ------------------------------------------------------------

当用户输入"不差钱……顶级 3A 内容"时，系统识别"不差钱"标签后将 \texttt{max\_price} 上限设为 999999，随即并发调用 \texttt{GPUAgent} 与 \texttt{GamingConsoleAgent}（逻辑参见 \texttt{ecosystem\_config.py}）。最终输出展示了 4090 显卡与高端电竞显示器的联动数据，LLM 生成的文案围绕"专业游戏"场景给出了完整的生态闭环建议，将单品推荐延伸为场景化组合方案。
![alt text](image.png)
...
参照 \texttt{test\_recommendation.py} 中的入门场景配置，系统对比了 5000 元价位段内索尼 ZV-E10 与尼康 Z fc 的视频能力（\texttt{Video\_Score}）。雷达图清晰勾勒出 ZV-E10 在视频防抖与对焦上的优势。对于初次选购相机的用户而言，这样的视觉呈现要比一张参数对比表更容易做出判断，也更契合该系统降低用户决策门槛的设计初衷。
![alt text](camera.png)
% ============================================================
\section{本章小结}
% ============================================================
...
多数推荐系统以单品类为设计边界，品类之间互不关联。然而消费者的实际采购需求往往并不孤立——刚入学的学生在选购笔记本时，多半同时需要耳机与移动电源；摄影爱好者在考虑升级机身的同时，镜头与存储方案的选配同样在决策范围之内。本文设计了\texttt{MultiCategoryAgent}路由模块与\texttt{EcosystemConfig}生态配置，赋予系统跨品类识别与联动推荐的能力。当用户的描述涉及"专业游戏"或"办公生产力"等场景语义时，系统可自动将相关品类纳入方案，将原本分散的选购环节整合为一次连贯的决策过程。
...
意图解析是推荐流程的入口，其质量直接决定后续环节的可用性。\texttt{CategoryDetector}在处理常规模糊表达时运行稳定，但在面对高度隐含前提的沟通方式时便会出现偏差。这类表达在日常对话中并不罕见，例如通过反讽传达偏好（"要那种不用一年就卡顿的手机"），或借助多重否定界定需求（"不要太贵但也别太便宜，拍照不用特别好，但也不能差"）。通用大模型对消费语境下的情感语义缺乏专项适配，面对上述情形时容易出现关键信息遗漏乃至方向性误判。引入经过细粒度情感分析微调的模型，是改善这一问题的可行思路，但需要结合具体数据集进行评估，并非一概而论地提升模型规模便能奏效。

\subsection{个体偏好的持续建模缺失}

系统当前的推荐权重来自人工预设的场景规则——\texttt{LaptopAgent}中，"学生场景"对轻薄与续航赋予较高权重，"专业创作场景"则侧重显示色域与处理性能。规则化方案逻辑清晰、易于审查，但只能反映群体层面的共性特征，对个体差异无能为力。用户对品牌的长期使用习惯、对外观设计的审美偏向，乃至对某类参数的特殊敏感，这些信息在当前架构中完全缺失。系统每次会话均从零开始，既不记录历史行为，也无法在多次交互中积累对特定用户的认知。在隐私保护前提下建立轻量级个人偏好模型，是系统走向实用化需要面对的实质性问题。

\subsection{后续改进方向}

基于上述分析，本文提出以下三个改进思路供后续参考。

\textbf{其一}，引入检索增强生成（RAG）以补充软性评价信息。当前推荐依据以硬件参数为主，缺乏对实际使用体验的呈现。而在消费者的决策过程中，握持手感、系统流畅度、售后口碑等非参数信息对最终选择的影响往往不亚于跑分数据。将Bilibili评测视频文字稿、什么值得买上的深度评测及用户评价纳入外挂知识库，通过RAG机制在参数比较之外同步检索真实使用反馈，可使推荐结果在"数据维度"之外兼顾"体验维度"。

\textbf{其二}，建立用户操作的反馈闭环。用户的每一次操作——重新生成方案、修改预算、反复对比某两款产品——都隐含着对当前推荐结果的评价信号。若能将这些信号系统性地收集并用于模型校准，系统便有条件从固定规则逐步过渡到基于真实交互数据的个性化策略。RLHF路径在技术层面已有相对成熟的参考实践，工程实施的难点在于如何设计合理的信号采集粒度与更新触发机制。

\textbf{其三}，将核心推荐引擎与Streamlit前端解耦。Streamlit适合快速验证原型，但在移动适配与分发灵活性上存在局限。若将推荐引擎以标准API形式独立部署，则可在此基础上分别接入微信小程序与多设备客户端。小程序无需安装、触达成本低；多设备流转则允许用户在不同终端间延续同一次咨询——手机上提问、电脑上深入比较、平板上查看可视化结果，各端衔接而不中断。
