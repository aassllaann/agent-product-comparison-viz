...
\begin{NKUAbstractZh}
  随着电子商务的快速发展，数码产品品类持续扩展，消费者在选购过程中面临日益严峻的信息过载问题。传统协同过滤方法依赖历史行为数据，在冷启动场景下表现
  不足；纯粹依赖大语言模型的对话式推荐易产生"幻觉"，缺乏可验证的事实依据；平台自带筛选功能则无法理解含混自然语言表达中的模糊需求。上述方法在自然语
  言理解、可靠事实约束与多维可视化辅助决策三项关键能力上均存在不同程度的缺失。

  针对上述问题，本文提出并实现了一套基于多智能体（Multi-Agent）架构的智能数码商品推荐系统。系统以大语言模型为语义核心，结合结构化关系型数据库作为 
...
\begin{figure}[h]
    ...
    \includegraphics[width=0.9\linewidth]{系统架构图.png} 
